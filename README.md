...
| Crate | Description |
|-------|-------------|
| [`devbox-common`](crates/devbox-common/) | Shared types: `DevboxId`, `DevboxState` enum, API request/response types |
...
| [`devbox-agent`](crates/devbox-agent/) | On-host binary baked into the golden AMI: `principals` (sshd resolver), `owner-sync` (provision the claimant's account), `warmup` (freshen `/workspace`, self-tag `devbox:ready=true`), `checkout` (clone repos into `/workspace`), `doctor` (diagnose warm-cache delivery) |
